#took like 8 mins to run. i'll run another test or two tomorrow but if it is consistently at or above the playwright approach im
#scrapping this one alltogether (i think network is really slow rn though so more tests are def in order)
#but: if i do, remember to fix the pllaywright version missing the last page (just add 1 to the max on the range)

#can make optimzation of combining url and req loops now that im not doing async
#7m 50s, internet is def still slow. maybe i'll do a playwright run and sync run to compare
#7m 50s
...
max_pages = re.search("Page 1 of (?P<max>\d+)", text)
...
#GET URLS AND RESPONSES
responses = []
for i in range(0, max_pages): #stop is exclusive, urls start at 0
    url = f"https://www.pmepensioen.nl/en/views/ajax?_wrapper_format=drupal_ajax&view_name=investments&view_display_id=shares&view_args=&view_path=%2Fnode%2F594&view_base_path=rest%2Fcontinent-investments&view_dom_id=5b380d85bc3f41b8e33a70d57fd946ac143fd842a84caebc7550ae957c7dd32e&pager_element=0&viewsreference%5Bcompressed%5D=eJxdkNsKgzAMQP8lzz5sDCfzZ0pGYw20VWp0iPjvS-lU2EMJyTm50A0sCkK7ASY3B4oCbZy9r6AntJSOzHNgRQAVDF030amN6C5LWDxBe9sroIhvT9aoKhzdlFccQ0o89V_c87CkFxh9LKuRdcxUi-gSjj38C2wV319Nc5GOyVsTMeTOkixMn0tItPDEQyzNzaOunwqLyULBWPL5Q277FzAaYdg&viewsreference%5Bcompressed%5D=eJxdkNsKgzAMQP8lzz5sDCfzZ0pGYw20VWp0iPjvS-lU2EMJyTm50A0sCkK7ASY3B4oCbZy9r6AntJSOzHNgRQAVDF030amN6C5LWDxBe9sroIhvT9aoKhzdlFccQ0o89V_c87CkFxh9LKuRdcxUi-gSjj38C2wV319Nc5GOyVsTMeTOkixMn0tItPDEQyzNzaOunwqLyULBWPL5Q277FzAaYdg&page={i}&_drupal_ajax=1&ajax_page_state%5Btheme%5D=whitelabel&ajax_page_state%5Btheme_token%5D=&ajax_page_state%5Blibraries%5D=eJxVjVsOwyAMBC9E8ZGipVjBBVILQ9Pcvmr6SPvjHc2u5CDzpKJM4Q1O0TA3aDKKbSiKP4wfi45QxBJHZ5t1rhRg7HrW6ZzQ6QMeZtxtLyI6CjZux1ZllUxf2p1pGUumV5zaqO4mvBrt1-OC-5-o1zgKuzVJ54LAhRCrLL_i-e4B4bdX4w"
    r = requests.get(url)
    if r and r.status_code == 200:
        print(i)
        responses.append(r)
...
p_match_entry = re.compile("!!!?(?P<issuer>[A-Za-z\d\-\. /&,;#]+)!!!(?P<value>[\d\.]+)!!!(?P<country>[A-Za-z]+(?: [A-Za-z ]+)?)(?:, Republic of)?!!!(?P<sector>[A-Za-z ,\-]+)!!!(?P<type>[A-Za-z ]+)")
...
with open(path/"raw_data_pme.txt", 'w') as file: #path
    for i, r in enumerate(responses):
        ...
        for match in matches:
            ...
            entry = [shareholder, issuer.strip(), country.strip(), sector.strip(), sectype.strip(), report_date, value.strip(), multiplier, currency, data_url]
